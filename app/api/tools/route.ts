...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await repoListToolNotes({
    ...
    const message = err instanceof Error ? err.message : "Lỗi tải tools";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = await repoUpsertToolNote({
    ...
    const message = err instanceof Error ? err.message : "Lỗi lưu tools";
    if (isMaxConnSessionError(err)) {
      return NextResponse.json({ error: message }, { status: 503 });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: "Thiếu id tools." }, { status: 400 });
    }
    const actorUsername =
      typeof body?.actorUsername === "string" ? body.actorUsername : undefined;
    const data = await repoCancelToolNote(id, actorUsername);
    return NextResponse.json({ data });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Lỗi hủy tools";
    if (isMaxConnSessionError(err)) {
      return NextResponse.json({ error: message }, { status: 503 });
    }
    return NextResponse.json({ error: message }, { status: 400 });
